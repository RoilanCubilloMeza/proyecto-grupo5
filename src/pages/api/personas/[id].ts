import { personaProvider } from "@/database/providers/personas/persona.provider";
...
async function getById(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await personaProvider.getById(id);
...
async function updateById(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { name } = req.body;
  try {
    await personaProvider.updateById(id, id { id, name });
...
async function deleteById(req: NextApiRequest, res: NextApiResponse) {
  const id = Number(req.query.id);
  try {
await personaProvider.deleteById(id);
